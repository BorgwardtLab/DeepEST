...
def update_scores(preds,go_fn):
    # Propagate scores with ontology structure
    go = Ontology(go_fn, with_rels=True)
    gos = list(preds.columns)
    to_add = {}
    for go_id in gos:
        for g_id in go.get_anchestors(go_id):
            if g_id in preds.columns:
                preds[g_id] = np.maximum(preds[g_id].values, preds[go_id].values)
            elif g_id in to_add.keys():
                to_add[g_id] = np.maximum(to_add[g_id], preds[go_id].values)
            else:
                to_add[g_id] = preds[go_id].values
    add_df = pd.DataFrame(to_add,index=preds.index)
    preds = pd.concat([preds,add_df],axis=1)
...
def preds_to_df(preds,df_template):
    df_go_terms = pd.DataFrame(data=0,index=df_template.index,columns=df_template.columns)
    for p in tqdm(preds):
        line = p.strip('\n').split('\t')
        idx = line.pop(0)
        if idx in df_go_terms.index:
            for l in line:
                term,pred = l.split('|')
                if term in df_go_terms.columns:
                    df_go_terms.loc[idx,term] = float(pred)
    return df_go_terms